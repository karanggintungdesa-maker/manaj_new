...
export const generateAttendancePDF = async (report: AttendanceReportData): Promise<Blob> => {
  ...
  const addLandscapeKop = () => {
    ...
    doc.setFontSize(14);
    doc.text("DESA KARANGGINTUNG", pageWidth / 2, 25, { align: "center" });
    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.text("Jl. Pelita Km 02, Desa Karanggintung, Kec. Gandrungmangu, Cilacap, Jawa Tengah", pageWidth / 2, 29, { align: "center" });
    doc.text("Tlp. 0852-1800-0668, Laman : www.karanggintung.id, Pos-el : desakaranggintung@gmail.com", pageWidth / 2, 33, { align: "center" });
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  const title = `ABSENSI PERANGKAT DESA KARANGGINTUNG BULAN ${monthName} ${year}`;
  ...
  doc.setFontSize(9);
  doc.text(`Karanggintung, ${footerDate}`, sigX, currentY);
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", sigX, currentY + 5);
  currentY += 22;
