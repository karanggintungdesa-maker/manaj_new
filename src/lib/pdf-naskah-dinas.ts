...
export const generateNaskahPDF = async (type: string, data: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  if (type === 'UND') {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(`Karanggintung, ${format(letterDate, "d MMMM yyyy", { locale: localeID })}`, pageWidth - margin, currentY, { align: "right" });
    ...
    const details = [
      ...
      { l: "Tempat", v: data.tempat || "Balai Desa Karanggintung" },
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text("KEPUTUSAN KEPALA DESA KARANGGINTUNG", pageWidth / 2, currentY, { align: "center" });
    currentY += lineH;
    ...
    doc.text("KEPALA DESA KARANGGINTUNG,", pageWidth / 2, currentY, { align: "center" });
    currentY += 10;
    ...
    const opening = `Pada hari ini ${dayName} tanggal ${terbilang(dateNum)} bulan ${monthName} tahun ${terbilang(yearNum)}, bertempat di ${data.tempat || "Balai Desa Karanggintung"}, telah dilaksanakan ${data.title || "kegiatan"}.`;
  ...
  if (type !== 'UND') {
    doc.text(`Karanggintung, ${footerDateStr}`, sigX, currentY);
    currentY += 5;
  }
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", sigX, currentY);
  currentY += 18; 
