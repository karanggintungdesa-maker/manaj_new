...
export const generatePhysicalDocPDF = async (type: string, data: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  if (type === 'prop_cover') {
    ...
    doc.setFontSize(16);
    doc.text("PEMERINTAH DESA KARANGGINTUNG", pageWidth / 2, 230, { align: "center" });
    doc.setFontSize(14);
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);
    const pendahuluan = `Dalam rangka meningkatkan infrastruktur dan kualitas hidup masyarakat di ${data.lokasi}, Pemerintah Desa Karanggintung memandang perlu melaksanakan ${data.namaKegiatan} melalui pemanfaatan ${data.sumberDana} Tahun Anggaran ${data.tahunAnggaran}.`;
    ...
    y += 45;
    doc.setFont("helvetica", "bold");
    doc.text("Menyetujui,\nKepala Desa Karanggintung", pageWidth / 2, y, { align: "center" });
    y += 25;
    doc.text((data.nama_kades || "TURMONO").toUpperCase(), pageWidth / 2, y, { align: "center" });
    const nW = doc.getTextWidth((data.nama_kades || "TURMONO").toUpperCase());
    ...
    const renderSurveyPage = (pageIdx: number) => {
        ...
        doc.setFontSize(10);
        doc.text("DESA", margin, 25); doc.text(`: KARANGGINTUNG`, margin + 30, 25);
        ...
        doc.setFontSize(10);
        y += 10;
        doc.text(`Karanggintung, ${dStr}`, sigCenterX, y, { align: "center" });
        y += 6;
    ...
    docL.setFont("helvetica", "normal");
    docL.text(`Desa Karanggintung, ${dStr}`, sX[2], y, { align: "center" });
    ...
    docL.text("TURMONO", sX[1], y + 10, { align: "center" });
    docL.line(sX[1] - docL.getTextWidth("TURMONO")/2, y + 10.8, sX[1] + docL.getTextWidth("TURMONO")/2, y + 10.8);
    ...
    const openingY = 85;
    const openingText = `Pada hari ini ${format(dBast, "EEEE", { locale: localeID })} tanggal ${terbilang(getDate(dBast))} bulan ${format(dBast, "MMMM", { locale: localeID })} tahun ${terbilang(getYear(dBast))} bertempat di Desa Karanggintung, kami yang bertanda tangan dibawah ini :`;
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    ...
    const alamat1 = `Desa Karanggintung Kecamatan Gandrungmangu yang selanjutnya disebut PIHAK KESATU`;
    ...
    doc.text("Nama", labelX, y); doc.text(":", dotX, y); doc.setFont("helvetica", "bold"); doc.text("TURMONO", valX, y); y += 6;
    doc.setFont("helvetica", "normal");
    doc.text("Jabatan", labelX, y); doc.text(":", dotX, y); doc.text("Pemegang Kekuasaan Pengelolaan Keuangan Desa", valX, y); y += 6;
    doc.text("Alamat", labelX, y); doc.text(":", dotX, y); 
    const alamat2 = `Desa Karanggintung Kecamatan Gandrungmangu yang selanjutnya disebut PIHAK KEDUA`;
    ...
    y += 25;
    doc.text(`Karanggintung, ${format(dBast, "d MMMM yyyy", { locale: localeID })}`, pageWidth - margin, y, { align: "right" }); y += 10;
    addSignatures(y, "PIHAK KEDUA\nPemegang Kekuasaan\nPengelolaan Keuangan Desa", "TURMONO", "PIHAK KESATU\nPelaksana Kegiatan Anggaran", data.nama_kasi);
  }
    ...
    doc.setFontSize(10);
    let y = 62;
    const bodyText = `Bahwa dalam pelaksanaan pembangunan Desa Karanggintung Kecamatan Gandrungmangu Kabupaten Cilacap Provinsi Jawa Tengah, maka pada :`;
    ...
    doc.text("Kepala Desa Karanggintung", pageWidth / 2, y, { align: "center" }); y += 18;
    doc.text("TURMONO", pageWidth / 2, y, { align: "center" });
    ...
    y += 30;
    doc.setFont("helvetica", "normal");
    doc.text(`Karanggintung, ${formattedDate}`, pageWidth - margin - 60, y);
    y += 6;
    doc.setFont("helvetica", "bold");
    doc.text("Ketua TPK Desa Karanggintung,", pageWidth - margin - 60, y);
    y += 28;
