...
export const addKopSuratSync = (doc: jsPDF, img: HTMLImageElement | null, margin: number, pageWidth: number) => {
    ...
    doc.setFontSize(16);
    doc.text("DESA KARANGGINTUNG", pageWidth / 2 + 10, 27, { align: "center" });
    doc.setFontSize(8);
    doc.setFont("helvetica", "normal");
    doc.text("Jl. Pelita Km 02, Desa Karanggintung, Kec. Gandrungmangu, Cilacap, Jawa Tengah", pageWidth / 2 + 10, 31, { align: "center" });
    doc.text("Tlp. 0852-1800-0668, Laman : www.karanggintung.id, Pos-el : desakaranggintung@gmail.com", pageWidth / 2 + 10, 35, { align: "center" });
    doc.setFont("helvetica", "bold");
...
export const generateBASTPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  const openingText = `Pada hari ini ${format(d, "EEEE", { locale: localeID })} tanggal ${terbilang(getDate(d))} bulan ${format(d, "MMMM", { locale: localeID })} tahun ${terbilang(getYear(d))} bertempat di Desa Karanggintung, kami yang bertanda tangan dibawah ini :`;
  ...
  const addIdentitas = (num: string, name: string, job: string, role: string) => {
    ...
    doc.text(":", dotX, currentY);
    const alamatFull = `Desa Karanggintung Kecamatan Gandrungmangu yang selanjutnya disebut ${role}`;
  ...
  addIdentitas("II", "TURMONO", "Pemegang Kekuasaan Pengelolaan Keuangan Desa", "PIHAK KEDUA");
  currentY += 2;
  ...
  currentY += rowH + 15;
  doc.text(`Karanggintung, ${format(d, "d MMMM yyyy", { locale: localeID })}`, pageWidth - margin, currentY, { align: "right" });
  ...
  doc.setFont("helvetica", "bold");
  doc.text("TURMONO", margin + 35, nameY, { align: "center" });
  const w1 = doc.getTextWidth("TURMONO");
...
export const generateSiltapPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  doc.setFont("helvetica", "normal");
  doc.text(`Karanggintung, ${formatDateIndo(values.date)}`, sigX, currentY);
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", sigX, currentY + 6);
  currentY += 25; 
  doc.text("TURMONO", sigX, currentY);
  const nW = doc.getTextWidth("TURMONO");
...
export const generateHonorNarasumberPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  addHeaderRow("Tempat", values.location || "Balai Desa Karanggintung");
...
export const generateSuratTugasPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  const drawDasar = () => {
    ...
    const items = values.dasar ? values.dasar.split("\n").filter(Boolean) : [
          ...
          "Peraturan Desa Karanggintung Nomor 13 Tahun 2025 tentang Anggaran Pendapatan dan Belanja Desa (APBDes) Tahun Anggaran 2026.",
        ];
...
const drawUntuk = () => {
  ...
  const drawSignature = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(`Karanggintung, ${format(d, "d MMMM yyyy", { locale: localeID })}`, sigX, currentY);
    doc.setFont("helvetica", "bold");
    doc.text("Kepala Desa Karanggintung,", sigX, currentY + 6);
    currentY += 28;
    doc.text("TURMONO", sigX, currentY);
    const width = doc.getTextWidth("TURMONO");
...
export const generateSPPDPDF = async (values: any, logoBase64?: string | null): Promise<Blob> => {
  ...
  addRow("1", "Pejabat berwenang yang memberi perintah", "Kepala Desa Karanggintung");
  ...
  addRow("6", "a. Tempat Berangkat", "Desa Karanggintung");
  ...
  addRow("9", "Pembebanan Anggaran", "APBDes Desa Karanggintung 2026");
  addRow("", "a. Instansi", "Pemerintah Desa Karanggintung");
  addRow("", "b. Akun", "Belanja Perjalanan Dinas");
  addRow("10", "Keterangan Lain-lain", "-");
  currentY += 15;
  const sigX = pageWidth - margin - 65;
  doc.text(`Dikeluarkan di : Karanggintung`, sigX, currentY);
  doc.text(`Pada Tanggal   : ${format(d, "d MMMM yyyy", { locale: localeID })}`, sigX, currentY + 5);
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", sigX, currentY + 12);
  currentY += 32;
  doc.text("TURMONO", sigX, currentY);
  const nW = doc.getTextWidth("TURMONO");
  doc.line(sigX, currentY + 1, sigX + nW, currentY + 1);
  ...
  doc.text("Berangkat dari : Desa Karanggintung", midX + 7, margin + 5);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", midX + colW2 / 2, margin + 27, { align: "center" });
  doc.text("TURMONO", midX + colW2 / 2, margin + 40, { align: "center" });
  const wH1 = doc.getTextWidth("TURMONO");
...
let y2 = margin + rowH2;
    ...
    doc.text(":", rDotX, y2 + 5);
    doc.text("Desa Karanggintung", rValueX, y2 + 5);
    ...
    doc.text("Desa Karanggintung", ivValueX, y2 + 5);
    doc.setFontSize(8);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Kepala Desa Karanggintung,", margin + colW2 / 2, y2 + 25, { align: "center" });
  doc.text("TURMONO", margin + colW2 / 2, y2 + 38, { align: "center" });
  const wH2 = doc.getTextWidth("TURMONO");
